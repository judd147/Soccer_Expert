...
def main():
    parser = GooeyParser()
    group1 = parser.add_argument_group('回测分析', gooey_options={"columns": 1})
    group1.add_argument('-file', metavar='数据库文件所在路径', widget="FileChooser")
    group1.add_argument('-path', metavar='结果文件保存路径', widget="DirChooser")
    group1.add_argument('--opt1', metavar='统计历史胜率', action='store_true')  
...
def search(df, args):
    ...
    if args.opt1:
        history = True
    #创建新工作簿以写入结果
    wb = xlsxwriter.Workbook(args.path+'\\result.xlsx')
